...
//
    ...
    //     if ($this->user_id > 0) {
    //         $x = 0;
    //         $products_found = \App\Models\Cart::where('user_id', $this->user_id)->pluck('product_id')->toArray();
    //         $all_products_edit = \App\Models\Cart::where('key_id', $this->firebase_token)->where('user_id', 0)->get();
    //         foreach ($all_products_edit as $one_product_edit) {
    //             if (!in_array($one_product_edit['product_id'], $products_found)) {
    //                 $one_product_edit->user_id = $this->user_id;
    //                 $one_product_edit->save();
    //                 $x = 1;
    //             } else {
    //                 $one_product_edit->delete();
    //             }
    //         }
    ...
    public function app_setting() {
        ...
        if ($one_ads) {
            ...
            $ads['category_id'] = $one_ads['category_id'];
            $ads['product_id'] = $one_ads['product_id'];
        } else {
            $ads = (object) [];
        }
        ...
        $response['data'] = ['setting' => $setting, 'currencies' => $currencies, 'ads' => $ads];
    ...
    public function ads() {
        ...
        $response['data'] = ['currencies' => $currencies, 'ads' => $ads];
        return Response()->json($response);
    }
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt(['email' => $request['email'], 'password' => $request['password'], 'deleted_at' => null], FALSE)) {
                ...
                if ($active == 'yes') {
                    ...
                    $products_found = \App\Models\Cart::where('user_id', Auth::User()->id)->pluck('product_id')->toArray();
                    $all_products_edit = \App\Models\Cart::where('key_id', $this->firebase_token)->where('user_id', 0)->get();
                    foreach ($all_products_edit as $one_product_edit) {
                        if (!in_array($one_product_edit['product_id'], $products_found)) {
                            $one_product_edit->user_id = Auth::User()->id;
                            $one_product_edit->save();
                        } else {
                            $one_product_edit->delete();
                        }
                    ...
                    $cart_count = 0;
                    $all_products = \App\Models\Cart::where('user_id', Auth::User()->id)->get();
                    foreach ($all_products as $one_product) {
                        if ($one_product['Product']['active'] == 'yes') {
                            if ($one_product['Product']['purchasable_type'] == 'limited' && $one_product['quantity'] > $one_product['Product']['purchasable']) {
                                $quantity = $one_product['Product']['purchasable'];
                    ...
                    $ios_data = ['action' => 'cart', 'count' => $cart_count, 'title' => '', 'alert' => '', 'image' => ''];
                    $android_data = ['action' => 'cart', 'count' => $cart_count, 'title' => '', 'message' => '', 'image' => ''];
                    $android_devices = \App\Models\ApiKey::where('user_id', Auth::User()->id)->where('platform', 'android')->pluck('key')->toArray();
                    $ios_devices = \App\Models\ApiKey::where('user_id', Auth::User()->id)->where('platform', 'ios')->pluck('key')->toArray();
    ...
    public function register_get() {
        ...
        $response['data'] = ['countries' => $countries, 'pages' => $pages];
        return Response()->json($response);
    }
    ...
    public function register_post() {
        ...
        if ($validator->fails()) {
            ...
            $products_found = \App\Models\Cart::where('user_id', $new->id)->pluck('product_id')->toArray();
            $all_products_edit = \App\Models\Cart::where('key_id', $this->firebase_token)->where('user_id', 0)->get();
            foreach ($all_products_edit as $one_product_edit) {
                if (!in_array($one_product_edit['product_id'], $products_found)) {
                    $one_product_edit->user_id = $new->id;
                    $one_product_edit->save();
                } else {
                    $one_product_edit->delete();
                }
            ...
            $cart_count = 0;
            $all_products = \App\Models\Cart::where('user_id', $new->id)->get();
            foreach ($all_products as $one_product) {
                if ($one_product['Product']['active'] == 'yes') {
                    if ($one_product['Product']['purchasable_type'] == 'limited' && $one_product['quantity'] > $one_product['Product']['purchasable']) {
                        $quantity = $one_product['Product']['purchasable'];
            ...
            $ios_data = ['action' => 'cart', 'count' => $cart_count, 'title' => '', 'alert' => '', 'image' => ''];
            $android_data = ['action' => 'cart', 'count' => $cart_count, 'title' => '', 'message' => '', 'image' => ''];
            $android_devices = \App\Models\ApiKey::where('user_id', $new->id)->where('platform', 'android')->pluck('key')->toArray();
            $ios_devices = \App\Models\ApiKey::where('user_id', $new->id)->where('platform', 'ios')->pluck('key')->toArray();
    ...
    public function home_screen() {
        ...
        $slider = [];
        $all_slider = \App\Models\Slider::where('type', 'app')->where('active', 'yes')->orderBy('sort', 'asc')->take(5)->get();
        foreach ($all_slider as $one_slider) {
            ...
            $one['category_id'] = $one_slider['category_id'];
            $one['product_id'] = $one_slider['product_id'];
            array_push($slider, $one);
        }

        $products = [];
        $all_products = \App\Models\Product::where('active', 'yes')->where('featured', 'yes')->orderBy('id', 'desc')->take(8)->get();
        foreach ($all_products as $one_product) {
            $price = \App\Models\ProductPrice::where('currency_id', $this->currency['id'])->where('product_id', $one_product['id'])->first();
        ...
        $whatsapp = \App\Models\Site::first()->whatsapp;

        $response['status'] = 1;
        $response['message'] = trans('api.home_screen');
        $response['data'] = ['slider' => $slider, 'categories' => $categories, 'products' => $products, 'whatsapp' => $whatsapp];
    ...
    public function products() {
        ...
        if (isset($request['sort']) && $request['sort'] != '') {
            if ($request['sort'] == 'a_z') {
                ...
                $all_products = $all_products->orderBy('products.id', 'desc');
            }
        } else {
            $all_products = $all_products->orderBy('products.id', 'desc');
        }
