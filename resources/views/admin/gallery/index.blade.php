...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active {{$pull}}">{{trans('admin.gallery')}}</li>
    </ol>
</section>
        ...
        <div class="col-12">
            ...
            <div class="box">
                <div class="box-header">
                    <a href="{{URL::to('admin/gallery/create')}}" class="btn btn-lg bg-black">{{trans('admin.gallery_add')}}</a>
                ...
                <div class="box-body">
                    <table id="{{$datatable}}" class="table table-bordered table-hover display nowrap margin-top-10 table-responsive">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                          <iframe width="150" height="150" src="{{ $one->file }}" frameborder="0" allowfullscreen>
                            </iframe>
                            </td>
                                
                                <td style="vertical-align: middle;">{{$one['ar_title']}}</td>
                                <td style="vertical-align: middle;">{{$one['en_title']}}</td>

                                <td style="vertical-align: middle;">
                                    {{ Form::open(array('url' =>'admin/gallery/'.$one->id.'/edit', 'method' => 'GET')) }}
                                    <button  type="submit" class="btn default btn-sm btn-info"><i class="fa fa-edit"></i> {{trans('admin.edit')}} </button>
                                    {{ Form::close() }}
                                </td>

                                <td style="vertical-align: middle;">
                                    <form action="{{route('gallery.destroy',$one->id)}}" method="POST">
                                        @method('DELETE')
                                        @csrf
...
<div class="modal fade" id="Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        {{ Form::open(array('url' =>'admin/delete_all/gallery', 'method' => 'POST')) }}
