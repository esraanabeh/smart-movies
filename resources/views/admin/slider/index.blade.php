...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active {{$pull}}">{{trans('admin.slider')}}</li>
    </ol>
</section>
        ...
        <div class="col-12">
            ...
            <div class="box">
                <div class="box-header">
                    <a href="{{URL::to('admin/slider/create')}}" class="btn btn-lg bg-black">{{trans('admin.slider_add')}}</a>
                    ...
                    <table id="{{$datatable}}" class="table table-bordered table-hover display nowrap margin-top-10 table-responsive">
                        <thead>
                            <tr>
                                ...
                                <th class="{{$text}}">{{trans('admin.slider_name_ar')}}</th>
                                <th class="{{$text}}">{{trans('admin.slider_name_en')}}</th>
                                <th class="{{$text}}">{{trans('admin.slider_sort')}}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><img class="img-thumbnail" style="height: 80px; width: 100px;" src="{{URL::to('upload/slider/'.$one['ar_image'])}}"></td>
                                {{-- <td><img class="img-thumbnail" style="height: 80px; width: 100px;" src="{{URL::to('upload/slider/'.$one['en_image'])}}"></td> --}}
                                <td style="vertical-align: middle;">{{$one['ar_name']}}</td>
                                <td style="vertical-align: middle;">{{$one['en_name']}}</td>
                                <td style="vertical-align: middle;"><span style="margin-right: 10px;" class="label label-info">{{$one->sort}}</span></td>
                                
                                <td style="vertical-align: middle;">
                                    <label class="switch" style="margin-bottom: 0;">
                                        @if($one['active'] == 'yes')
                                        <input class="switch_active" page='slider' id="{{$one['id']}}" type="checkbox" checked="">
                                        @else
                                        <input class="switch_active" page='slider' id="{{$one['id']}}" type="checkbox">
                                        @endif
                                        <span class="slider"></span>
                                    </label>
                                </td>
                                <td style="vertical-align: middle;">
                                    {{ Form::open(array('url' =>'admin/slider/'.$one->id.'/edit', 'method' => 'GET')) }}
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    {{ Form::open(array('url' =>'admin/slider/'.$one->id, 'method' => 'DELETE')) }}
                                ...
                                <td style="vertical-align: middle;">
                                    <form action="{{route('slider.destroy',$one->id)}}" method="POST">
                                        @method('DELETE')
                                        @csrf
...
<div class="modal fade" id="Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        {{ Form::open(array('url' =>'admin/delete_all/slider', 'method' => 'POST')) }}
        <div class="modal-content">
            <div class="modal-header">
