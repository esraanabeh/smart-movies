...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active {{$pull}}">{{trans('admin.photo')}}</li>
    </ol>
</section>
        ...
        <div class="col-12">
            ...
            <div class="box">
                <div class="box-header">
                    <a href="{{URL::to('admin/photo/create')}}" class="btn btn-lg bg-black">{{trans('admin.photo_add')}}</a>
                 ...
                 <div class="box-body">
                    <table id="{{$datatable}}" class="table table-bordered table-hover display nowrap margin-top-10 table-responsive">
                        <thead>
                            <tr>
                                <th class="{{$text}}">#</th>
                                <th class="{{$text}}">{{trans('admin.photo')}}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td> <img src="{{URL::to('photo/'.$one['photo'])}}" style="height: 80px; width: 100px;"/></td>
                                
                                <td>
                                    {{ Form::open(array('url' =>'admin/photo/'.$one->id.'/edit', 'method' => 'GET')) }}
                                    <button  type="submit" class="btn default btn-sm btn-info"><i class="fa fa-edit"></i> {{trans('admin.edit')}} </button>
                                    {{ Form::close() }}
                                </td>
                             
                                <td>
                                    <form action="{{route('photo.destroy',$one->id)}}" method="POST">
                                        @method('DELETE')
                                        @csrf
...
<div class="modal fade" id="Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        {{ Form::open(array('url' =>'admin/delete_all/photo', 'method' => 'POST')) }}
